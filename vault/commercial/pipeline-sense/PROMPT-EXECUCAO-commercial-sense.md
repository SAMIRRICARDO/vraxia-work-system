...
═══════════════════════════════════════════════════════════
ATUALIZAÇÃO — LEAD INTELLIGENCE 360°
Adicionar após o arquivo 13 (enrichment.ts):

13b. prompts/commercial/leadIntelligencePrompt.ts
     Três prompts exportados:
     BEHAVIORAL_ANALYSIS_PROMPT (max 100 tokens output)
     STRATEGIC_ANALYSIS_PROMPT (max 100 tokens output)
     INTELLIGENCE_SYNTHESIS_PROMPT (max 300 tokens output, Sonnet)
     (ver seção 5 de lead-intelligence-spec.md)

13c. agents/commercial/leadIntelligence.ts
     runLeadIntelligence360() — 3 chamadas sequenciais:
     Chamada 1: Haiku, behavioral, max_tokens: 100
     Chamada 2: Haiku, strategic, max_tokens: 100
     Chamada 3: Sonnet, synthesis, max_tokens: 300
     Substitui enrichment.ts + scoring.ts no pipeline
     (ver seção 6 de lead-intelligence-spec.md)

ATUALIZAR workers/executionQueue.ts:
Trocar steps 'enrich_lead' + 'score_lead' por 'lead_intelligence_360'
Adicionar 'lead_intelligence_360' no AGENT_MAP apontando para runLeadIntelligence360

PIPELINE ATUALIZADO:
search_lead → lead_intelligence_360 → generate_outreach → crm
═══════════════════════════════════════════════════════════
